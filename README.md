...
- **App Privacy ("data usages") can't be automated here — set it manually, once.** A `declare_no_data_collection` lane was added to publish the "Data Not Collected" declaration via `Spaceship::ConnectAPI::AppDataUsagesPublishState`, but the installed `fastlane-2.237.0`'s endpoint for this (`GET /v1/apps/{id}/dataUsagePublishState`, in `spaceship/lib/spaceship/connect_api/tunes/tunes.rb:304`) is rejected by the live App Store Connect API with `The URL path is not valid - The relationship 'dataUsagePublishState' does not exist` — that relationship has moved or been renamed server-side since this fastlane version was cut, and no amount of correct arguments on our end route around a stale path baked into the gem. Rather than leave a lane in the critical path that can never succeed, it was removed. **One-time manual step:** in App Store Connect → App Privacy, answer "No, we do not collect data from this app" and publish (true per `docs/privacy.html`) before the first submission. Once published it, like the other fields above, doesn't need repeating on future releases.
